...
var _resolveSearchConfig = _interopRequireDefault(require("../resolveSearchConfig"));
...
var OVERRIDABLE_FIELDS = ['jsonType', 'orderings', 'type', 'name', 'title', 'readOnly', 'hidden', 'description', '__experimental_search', 'options', 'inputComponent', 'validation', 'icon'];

var normalizeSearchConfig = configs => {
...
var ObjectType = {
  ...
  extend(rawSubTypeDef, createMemberType) {
    ...
    (0, _utils.lazyGetter)(parsed, '__experimental_search', () => {
      var userProvidedSearchConfig = subTypeDef.__experimental_search ? normalizeSearchConfig(subTypeDef.__experimental_search) : null;

      if (userProvidedSearchConfig) {
        return userProvidedSearchConfig.map(entry => entry === 'defaults' ? (0, _resolveSearchConfig.default)(subTypeDef) : entry);
      }

      return (0, _resolveSearchConfig.default)(parsed);
    }, {
      enumerable: false
    });
