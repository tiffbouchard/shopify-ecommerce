    ...
    return function withEditableAPI(editor) {
        ...
        portableTextEditor.setEditable({
            ...
            focusBlock: () => {
                if (editor.selection) {
                    ...
                    try {
                        ...
                        if (block) {
                            return values_1.fromSlateValue([block], portableTextFeatures.types.block.name)[0];
                        }
                    }
                    catch (err) {
                        return undefined;
                ...
                if (editor.selection) {
                    try {
                        ...
                        if (node && !slate_1.Editor.isBlock(editor, node)) {
                            ...
                            return values_1.fromSlateValue([pseudoBlock], portableTextFeatures.types.block.name)[0]
                                .children[0];
                        }
                    }
                    catch (err) {
            ...
            findByPath: (path) => {
                ...
                if (slatePath) {
                    ...
                    if (block && blockPath && typeof block._key === 'string') {
                        ...
                        const ptBlock = values_1.fromSlateValue([block], portableTextFeatures.types.block.name)[0];
            ...
            activeAnnotations: () => {
                ...
                try {
                    const [block] = slate_1.Editor.node(editor, editor.selection.focus.path.slice(0, 1));
                    if (!Array.isArray(block.markDefs)) {
                        return [];
                    }
                    const [span] = slate_1.Editor.node(editor, editor.selection.focus.path.slice(0, 2));
                    return block.markDefs.filter(def => Array.isArray(span.marks) && span.marks.includes(def._key));
                }
                catch (err) {
                    return [];
                }
            ...
            removeAnnotation: () => {
                ...
                if (selection) {
                    const [blockElement] = slate_1.Editor.node(editor, selection.focus, { depth: 1 });
                    if (blockElement._type === portableTextFeatures.types.block.name &&
                        slate_1.Element.isElement(blockElement)) {
                        const [span, spanPath] = slate_1.Editor.node(editor, selection.focus.path.slice(0, 2));
                        const spanMarks = span.marks || [];
                        if (Array.isArray(blockElement.markDefs)) {
                            if (slate_1.Range.isCollapsed(selection) &&
                                slate_1.Text.isText(span) &&
                                ...
                                slate_1.Transforms.select(editor, slate_1.Path.next(spanPath));
            ...
            getValue: () => {
                return values_1.fromSlateValue(editor.children, portableTextFeatures.types.block.name);
            }
        });
        return editor;
    };
