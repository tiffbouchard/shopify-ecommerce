...
exports.PortableTextEditable = (props) => {
    ...
    // Default value
    values_1.toSlateValue(getValueOrIntitialValue(value, [createPlaceHolderBlock()]), portableTextFeatures.types.block.name));
    ...
    const setValueFromProps = () => {
        if (VALUE_TO_SLATE_VALUE.get(value || []) !== stateValue) {
            ...
            const slateValueFromProps = values_1.toSlateValue(value, portableTextFeatures.types.block.name);
        ...
        if (onPaste) {
            ...
            return resolved
                ...
                if (typeof result === 'object' && result.insert) {
                    ...
                    const string = JSON.stringify(values_1.toSlateValue(blocksToInsertNormalized, portableTextFeatures.types.block.name));
    ...
    const slateEditable = react_1.useMemo(() => (react_1.default.createElement(slate_react_1.Slate, { onChange: handleChange, editor: editor, selection: selection, value: getValueOrIntitialValue(stateValue, [createPlaceHolderBlock()]) },
        react_1.default.createElement(slate_react_1.Editable, { className: 'pt-editable', autoFocus: false, 
            // decorate={decorate}
            onPaste: handlePaste, onCopy: handleCopy, onCut: handleCut, onSelect: handleSelect, onFocus: handleOnFocus, onBlur: handleOnBlur, onKeyDown: editor.pteWithHotKeys, placeholder: placeholderText, readOnly: readOnly, renderElement: renderElement, renderLeaf: renderLeaf, spellCheck: spellCheck }))), [placeholderText, readOnly, spellCheck, stateValue, selection]);
