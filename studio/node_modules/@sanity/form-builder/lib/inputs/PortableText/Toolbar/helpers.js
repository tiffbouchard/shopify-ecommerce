...
function getFormatIcon(type, schemaIcon) {
  switch (type) {
    ...
    default:
      return schemaIcon || _sanityLogoIcon.default;
  }
}
...
function getPTEListActions(editor, selection) {
  ...
  return features.lists.map(listItem => {
    var active = focusBlock ? focusBlock.listItem === listItem.value : false;
    return {
      active,
      key: listItem.value,
      disabled: !selection || (focusBlock ? features.types.block.name !== focusBlock._type : false),
...
function getPTEAnnotationActions(editor, onFocus) {
  var features = _portableTextEditor.PortableTextEditor.getPortableTextFeatures(editor);
  ...
  return features.annotations.map(item => {
    ...
    return {
      ...
      handle: () => {
        ...
        var paths = _portableTextEditor.PortableTextEditor.addAnnotation(editor, item.type);
...
function getPTEToolbarActionGroups(editor, selection, onFocus, hotkeyOpts) {
  return [{
    name: 'format',
    ...
    name: 'annotation',
    actions: getPTEAnnotationActions(editor, onFocus)
  }];
}
...
function getInsertMenuItems(editor, selection, onFocus) {
  var focusBlock = _portableTextEditor.PortableTextEditor.focusBlock(editor);
  ...
  var blockItems = features.types.blockObjects.map((type, index) => ({
    ...
    handle() {
      var path = _portableTextEditor.PortableTextEditor.insertBlock(editor, type);
  ...
  var inlineItems = features.types.inlineObjects.map((type, index) => ({
    ...
    handle() {
      var path = _portableTextEditor.PortableTextEditor.insertChild(editor, type);
  ...
  var filteredBlockItems = blockItems.concat(inlineItems).filter(item => {
    ...
    return !((_item$type = item.type) === null || _item$type === void 0 ? void 0 : _item$type.hidden);
  });
